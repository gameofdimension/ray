      ...
      R"(
{
  ...
  ASSERT_EQ(fake_ray_event_recorder_->GetBuffer().size(), 2);
  ASSERT_EQ(fake_ray_event_recorder_->GetBuffer()[0]->GetEventType(),
            rpc::events::RayEvent::DRIVER_JOB_DEFINITION_EVENT);
  ASSERT_EQ(fake_ray_event_recorder_->GetBuffer()[1]->GetEventType(),
